...
TEST(UnsortedArrayList_Duplicate_OrderPreserving, PushAndPopFrontTest)
{
	...
	UnsortedArrayList<std::string> slist;
		ASSERT_THROW(slist.popFront(), std::runtime_error);
		slist.pushFront("apple"s);
		slist.pushFront("grape"s);
...
TEST(UnsortedArrayList_Duplicate_OrderPreserving, listInitializationTest)
{
	...
	UnsortedArrayList<std::string> slist2{"kiwi"s,"apple"s,"melon"s,"mango"s,"banana"s,"grape"s};
	output = "";
	while(!slist2.isEmpty())
		output += slist2.popFront()+",";
