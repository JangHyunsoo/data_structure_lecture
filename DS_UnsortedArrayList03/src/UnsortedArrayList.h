 ...
 * 용량 고정, 중복 허용, 배열을 이용한 비정렬 범용 리스트
...
private:
	...
	size_t search(const T& item, size_t startLoc=0) const noexcept{
...
public:
	explicit UnsortedArrayList(size_t capacity = 5) : capacity{capacity}, items{new T[capacity]}{}
	...
	bool find(const T& item) const noexcept{
		if(isEmpty()) return false;
		return search(item)!=capacity;
	}

	void removeFirst(const T& item) noexcept{
		if(isEmpty()) return;
		size_t itemLoc{search(item)};
		if(itemLoc!=capacity) shiftLeft(itemLoc);
	}

	void removeAll(const T& item) noexcept{
		if(isEmpty()) return;
		size_t itemLoc{search(item)};
	...
	void insert(const T& index, int item){
		if(numItems==capacity) increaseCapacity();
	...
	auto begin() {return ListIterator<T>(items);}
	auto end() {return ListIterator<T>(items+numItems);}
	auto cbegin() const {return ListIterator<const T>(items);}
	auto cend() const {return ListIterator<const T>(items+numItems);}
};
