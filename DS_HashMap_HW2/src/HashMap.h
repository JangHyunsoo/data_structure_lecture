...
public:
	...
	void put(const K& key, const V& value){
		...
		if(containsKey(key)){
			...
			auto iter = getIterator(index, key);
			iter->second = value;
		}else{
			size_t index{getIndex(key)};
	...
	bool containsKey(const K& key) const noexcept{
		...
		size_t index{getIndex(key)};
		auto iter = getIterator(index, key);
		return iter!=table[index].end();
	}

	const V& get(const K& key) const{
		if(isEmpty()) throw std::runtime_error("HashMap: get, key does not exists");
		size_t index{getIndex(key)};
		auto iter = getIterator(index, key);
		if(iter == table[index].end()) throw std::runtime_error("HashMap: get, key does not exists");
		return iter->second;
	}
