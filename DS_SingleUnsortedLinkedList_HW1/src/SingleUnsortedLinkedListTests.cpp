...
TEST(SingleUnsortedLinkedList_Duplicate, listEmptyInitializationTest)
{
...
TEST(SingleUnsortedLinkedList_Duplicate, PushAndPopBackTest)
{
	SingleUnsortedLinkedList list;
	list.pushBack(3);
	list.pushBack(5);
...
TEST(SingleUnsortedLinkedList_Duplicate, PushAndPopBackTest_EXTRA)
{
	SingleUnsortedLinkedList list;
...
TEST(SingleUnsortedLinkedList_Duplicate, PushAndPopFrontTest)
{
	SingleUnsortedLinkedList list;
...
TEST(SingleUnsortedLinkedList_Duplicate, PushPopBackFrontTest)
{
	SingleUnsortedLinkedList list;
	list.pushFront(3);
...
TEST(SingleUnsortedLinkedList_Duplicate, listInitializationTest)
{
...
TEST(SingleUnsortedLinkedList_Duplicate, findTest){
...
TEST(SingleUnsortedLinkedList_Duplicate, removeFirstTest){
	SingleUnsortedLinkedList list;
	list.pushBack(3);
	list.pushBack(5);
	list.pushBack(7);
...
TEST(SingleUnsortedLinkedList_Duplicate, removeAllTest){
...
TEST(SingleUnsortedLinkedList_Duplicate, iteratorTest){
	SingleUnsortedLinkedList list;
	list.pushBack(3);
	list.pushBack(5);
	list.pushBack(7);
...
TEST(SingleUnsortedLinkedList_Duplicate, IndexOperator){
	SingleUnsortedLinkedList list;
	list.pushBack(3);
	list.pushBack(5);
	ASSERT_EQ(list[1], 5);
...
TEST(SingleUnsortedLinkedList_Duplicate, clearTest){
	SingleUnsortedLinkedList list{1,3,1,1,3,4,5,4,4,6};
	list.clear();
	ASSERT_TRUE(list.isEmpty());
	list.pushBack(4);
...
TEST(SingleUnsortedLinkedList_Duplicate, Big5)
{
...
TEST(SingleUnsortedLinkedList_Duplicate, Big5_peekBackTest)
{
	SingleUnsortedLinkedList list1{1,2,3,4,5};
	SingleUnsortedLinkedList list2{6,7,8,9,10};
	SingleUnsortedLinkedList list3;
	...
	for(int i=0; i<list2.size(); i++)
		ASSERT_EQ(list2[i], list4[i]);
	ASSERT_EQ(list2.peekBack(), list4.peekBack());
	list3 = list1;
	// copy assignment test
	for(int i=0; i<list1.size(); i++)
		ASSERT_EQ(list1[i], list3[i]);
	ASSERT_EQ(list1.peekBack(), list3.peekBack());
	// move constructor test
	SingleUnsortedLinkedList list5(std::move(list3));
	for(int i=0; i<list5.size(); i++)
		ASSERT_EQ(list5[i], list1[i]);
	ASSERT_EQ(list5.peekBack(), list1.peekBack());
	// move assignment test
	list3 = std::move(list4);
	for(int i=0; i<list3.size(); i++)
		ASSERT_EQ(list3[i], list2[i]);
	ASSERT_EQ(list3.peekBack(), list2.peekBack());
}

TEST(SingleUnsortedLinkedList_Duplicate, checkTail){
	SingleUnsortedLinkedList list;
	list.pushBack(1);
	list.removeFirst(1);
	ASSERT_THROW(list.peekBack(), std::runtime_error);
	list.pushBack(1);
	list.removeAll(1);
	ASSERT_THROW(list.peekBack(), std::runtime_error);
	// pushFront test (empty case)
	list.pushFront(1);
	ASSERT_EQ(list.peekBack(),1);
	// popFront test (empty case)
	ASSERT_EQ(list.popFront(),1);
	ASSERT_FALSE(list.checkTail());
	ASSERT_THROW(list.peekBack(), std::runtime_error);
	// pushBack test
	list.pushBack(1);
	list.pushBack(2);
